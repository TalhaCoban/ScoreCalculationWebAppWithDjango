...
def SaveResults(request,id):
    ...
    results = YKS(
        ogrenci = user,
        tytyer = temporaryDictionary[id]["tyt_yer"],
        tytham = temporaryDictionary[id]["tyt_ham"],
        sayisalham = temporaryDictionary[id]["ayt_sayisal_ham"],
        sayisalyer = temporaryDictionary[id]["ayt_sayisal_yer"],
        eaham = temporaryDictionary[id]["ayt_ea_ham"],
        eayer = temporaryDictionary[id]["ayt_ea_yer"],
        sozelham = temporaryDictionary[id]["ayt_sozel_ham"],
        sozelyer = temporaryDictionary[id]["ayt_sozel_yer"],
        ydtham = temporaryDictionary[id]["ydt_ham"],
        ydtyer = temporaryDictionary[id]["ydt_yer"],
        tyt_sir_2018_ham = temporaryDictionary[id]["siralamalar"]["tyt_sir_2018_ham"],
        tyt_sir_2019_ham = temporaryDictionary[id]["siralamalar"]["tyt_sir_2019_ham"],
        tyt_sir_2018_yer = temporaryDictionary[id]["siralamalar"]["tyt_sir_2018_yer"],
        tyt_sir_2019_yer = temporaryDictionary[id]["siralamalar"]["tyt_sir_2019_yer"],
        ayt_sir_sayisal_2018_ham = temporaryDictionary[id]["siralamalar"]["ayt_sir_sayisal_2018_ham"],
        ayt_sir_sayisal_2019_ham = temporaryDictionary[id]["siralamalar"]["ayt_sir_sayisal_2019_ham"],
        ayt_sir_sayisal_2018_yer = temporaryDictionary[id]["siralamalar"]["ayt_sir_sayisal_2018_yer"],
        ayt_sir_sayisal_2019_yer = temporaryDictionary[id]["siralamalar"]["ayt_sir_sayisal_2019_yer"],
        ayt_sir_ea_2018_ham = temporaryDictionary[id]["siralamalar"]["ayt_sir_ea_2018_ham"],
        ayt_sir_ea_2019_ham = temporaryDictionary[id]["siralamalar"]["ayt_sir_ea_2019_ham"],
        ayt_sir_ea_2018_yer = temporaryDictionary[id]["siralamalar"]["ayt_sir_ea_2018_yer"],
        ayt_sir_ea_2019_yer = temporaryDictionary[id]["siralamalar"]["ayt_sir_ea_2019_yer"],
        ayt_sir_sozel_2018_ham = temporaryDictionary[id]["siralamalar"]["ayt_sir_sozel_2018_ham"],
        ayt_sir_sozel_2019_ham = temporaryDictionary[id]["siralamalar"]["ayt_sir_sozel_2019_ham"],
        ayt_sir_sozel_2018_yer = temporaryDictionary[id]["siralamalar"]["ayt_sir_sozel_2018_yer"],
        ayt_sir_sozel_2019_yer = temporaryDictionary[id]["siralamalar"]["ayt_sir_sozel_2019_yer"],
        ydt_sir_2018_ham = temporaryDictionary[id]["siralamalar"]["ydt_sir_2018_ham"],
        ydt_sir_2019_ham = temporaryDictionary[id]["siralamalar"]["ydt_sir_2019_ham"],
        ydt_sir_2018_yer = temporaryDictionary[id]["siralamalar"]["ydt_sir_2018_yer"],
        ydt_sir_2019_yer = temporaryDictionary[id]["siralamalar"]["ydt_sir_2019_yer"]
    )
    results.save()
    messages.success(request, message="Puanlar Başarıyla Kaydedildi")
    return redirect(reverse("YKS:dashboard",kwargs={"id":id}))


def dashboard(request, id): 
    results = YKS.objects.filter(ogrenci_id = id)
    forGraph = request.GET.get("forGraph")
    ...
    if forGraph:
        if forGraph == "tyt":
            for result in results.order_by("created_date"):
                dataHam.append(float(result.tytham))
                dataYer.append(float(result.tytyer))
                labels.append(str(result.created_date.day)+'/'+str(result.created_date.month)+'/'+str(result.created_date.year))
        elif forGraph == "sayisal":
            for result in results.order_by("created_date"):
                dataHam.append(float(result.sayisalham))
                dataYer.append(float(result.sayisalyer))
                labels.append(str(result.created_date.day)+'/'+str(result.created_date.month)+'/'+str(result.created_date.year))
        elif forGraph == "ea":
            for result in results.order_by("created_date"):
                dataHam.append(float(result.eaham))
                dataYer.append(float(result.eayer))
                labels.append(str(result.created_date.day)+'/'+str(result.created_date.month)+'/'+str(result.created_date.year))
        elif forGraph == "sozel":
            for result in results.order_by("created_date"):
                dataHam.append(float(result.sozelham))
                dataYer.append(float(result.sozelyer))
                labels.append(str(result.created_date.day)+'/'+str(result.created_date.month)+'/'+str(result.created_date.year))
        else:
            for result in results.order_by("created_date"):
                dataHam.append(float(result.ydtham))
                dataYer.append(float(result.ydtyer))
                labels.append(str(result.created_date.day)+'/'+str(result.created_date.month)+'/'+str(result.created_date.year))
        context = {
            "results" : results,
            "dataHam" : dataHam,
        ...
        return render(request, "chartForYKS.html", context)
    else:
        context = {
            "results" : results,
        }
        return render(request, "dashboardForYKS.html", context)
...
def SureToDeleteResult(request,id):
    user_id = YKS.objects.get(id = id).ogrenci_id
    result = get_object_or_404(YKS, id = id)
    result.delete()
    return redirect(reverse("YKS:dashboard",kwargs={"id":user_id}))    


def detail(request,id):
    results = get_object_or_404(YKS, id = id)
    context = {
        "results" : results,
    }
